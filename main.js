...
function handleCareerExpand() {
  ...
  if (careerAction.textContent === "Read") {
    ...
    Array.from(careerReadHide).forEach(
    ...
    Array.from(careerReadHide).forEach((element) =>
...
function setupCoverflow() {
  let coverflowContainers = Array.from(
...
const changeImagePosition = (coverFlowPosition, images) => {
  ...
  const transformXNext = generateOffsetOrSizeArray(
    offsetRight,
    step,
    images.length
  );
  const transformXPrev = generateOffsetOrSizeArray(
    offsetLeft,
    step,
    images.length
  );
...
function sendEmail() {
  ...
  const mailToLink = `mailto:hrybkova.hanna@gmail.com?subject=${encodeURIComponent(
    subject
  )}&body=${encodeURIComponent(textWithName)}`;
